...
class Commodity(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        one_year_ago = timezone.now() -timezone.timedelta(days=1)
        seven_days_ago = timezone.now() -timezone.timedelta(days=7)


        previous_commodity_price = Commodity.objects.filter(name=self.name, time__lte=one_hour_ago).order_by('-time').first()

        if previous_commodity_price:
            self.price_1_hour_ago =previous_commodity_price.current_price
